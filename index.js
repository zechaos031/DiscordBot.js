...
client.on('guildMemberAdd', async member => {
	...
	ctx.fillText(`Bienvenue dans ${member.guild.name},`, canvas.width / 2.5, canvas.height / 3.5);

	ctx.font = applyText(canvas, `${member.displayName}#${member.user.discriminator}`);
	ctx.fillStyle = '#ffffff';
    ctx.fillText(`${member.displayName}#${member.user.discriminator}`, canvas.width / 2.5, canvas.height / 1.8);
    
    ctx.font = '25px sans-serif';
	ctx.fillStyle = '#ffffff';
	ctx.fillText(`On est maintenant ${member.guild.memberCount} membres !`, canvas.width / 2.5, canvas.height / 1.3);
...
client.on('guildMemberRemove', async member => {
    ...
    ctx.font = applyText(canvas, `${member.displayName}#${member.user.discriminator} a quitté ${member.guild.name}`);
	ctx.fillStyle = '#ffffff';
	ctx.fillText(`${member.displayName}#${member.user.discriminator} a quitté ${member.guild.name}`, canvas.width / 2.5, canvas.height / 2.5);
    
    ctx.font = '25px sans-serif';
	ctx.fillStyle = '#ffffff';
	ctx.fillText(`On est maintenant ${member.guild.memberCount} membres !`, canvas.width / 2.5, canvas.height / 1.6);
